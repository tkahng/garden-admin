...
function ReviewDialog({
  ...
  const mutation = useMutation({
    mutationFn: async (body: ReviewReturnRequest | undefined) => {
      ...
      const { error } = await apiClient.POST(path as "/api/v1/admin/returns/{id}/approve", {
        ...
        <Table>
          ...
          <TableBody>
            ...
            {!isLoading && returns.length === 0 && (
              <TableRow>
                <TableCell colSpan={6} className="py-12 text-center text-muted-foreground">No return requests.</TableCell>
              </TableRow>
            )}
            {returns.map((rr) => (
              <ReturnRow
